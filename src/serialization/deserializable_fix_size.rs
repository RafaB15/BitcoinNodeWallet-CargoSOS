...
{
    fn deserialize_fix_size(stream: &mut dyn Read, sizes: usize) -> Result<Self, ErrorSerialization>;
}
...
impl DeserializableFixSize for String {
    ...
    fn deserialize_fix_size(stream: &mut dyn Read, sizes: usize) -> Result<Self, ErrorSerialization> {
        ...
        if stream.read_exact(&mut buffer).is_err() {
            return Err(ErrorSerialization::ErrorInDeserialization("Deserializing string".to_string()));
        }

        match String::from_utf8(buffer.to_vec()) {
            Ok(string) => Ok(string),
            Err(_) => Err(ErrorSerialization::ErrorInDeserialization(format!("Converting from utf8 to string: {:?}", buffer))),
        }
    }
}
