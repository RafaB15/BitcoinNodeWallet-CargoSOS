...
use crate::connections::network_ip_address::NetworkIpAddress;
...
impl DeserializableInternalOrder for AddrMessage {
    fn io_deserialize(stream: &mut dyn Read) -> Result<Self, ErrorSerialization> {
        ...
        let mut ip_addresses: Vec<NetworkIpAddress> = Vec::new();

        for _ in 0..ip_address_count {
            ip_addresses.push(NetworkIpAddress::le_deserialize(stream)?);
        }
