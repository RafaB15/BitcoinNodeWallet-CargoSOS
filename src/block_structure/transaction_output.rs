...
mod tests {
    ...
    fn test_01_correct_transaction_output_serialization() {
        ...
        CompactSize::new(pk_script.len() as u64)
            .le_serialize(&mut fields_serialized)
            .unwrap();
        pk_script.io_serialize(&mut fields_serialized).unwrap();

        let mut output_serialized = Vec::new();
        transaction_output
            .io_serialize(&mut output_serialized)
            .unwrap();
    ...
    fn test_02_correct_transaction_output_deserialization() {
        ...
        let deserialized_transaction_output =
            TransactionOutput::io_deserialize(&mut &transaction_output_bytes[..]).unwrap();
