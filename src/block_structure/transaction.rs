...
mod tests {
    ...
    fn test_01_correct_transaction_serialization() {
        ...
        CompactSize::new(1 as u64).le_serialize(&mut expected_buffer).unwrap();
        transaction_input.io_serialize(&mut expected_buffer).unwrap();
        CompactSize::new(1 as u64).le_serialize(&mut expected_buffer).unwrap();
        transaction_output.io_serialize(&mut expected_buffer).unwrap();
        let time: u32 = 0;
    ...
    fn test_03_correct_tx_id() {
        ...
        let actual_tx_id: [u8; 32] = [45, 65, 106, 164, 25, 161, 226, 124, 62, 182, 123, 231, 104, 161, 13, 97, 30, 173, 224, 101, 171, 48, 77, 39, 87, 58, 123, 30, 225, 42, 175, 56];
    ...
    fn test_03_correct_verification_of_transaction_ownership() {
        ...
        let transaction_bytes: Vec<u8> = vec![0x01, 0x00, 0x00, 0x00, 0x01, 0x20, 0x25, 0xEF, 0x69, 0x2C, 0xA9, 0x87, 0xB3, 0x9A, 0x81, 0x33, 0x6E, 0xFB, 0x59, 0xB0, 0x56, 0xFB, 0x90, 0xC0, 0x3A, 0x5E, 0xA4, 0xC4, 0x54, 0x4C, 0xF9, 0x27, 0x57, 0x61, 0x3E, 0x2E, 0xA4, 0x01, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0x02, 0xA0, 0x86, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x19, 0x76, 0xA9, 0x14, 0x7A, 0xA8, 0x18, 0x46, 0x85, 0xCA, 0x1F, 0x06, 0xF5, 0x43, 0xB6, 0x4A, 0x50, 0x2E, 0xB3, 0xB6, 0x13, 0x5D, 0x67, 0x20, 0x88, 0xAC, 0xD0, 0xE6, 0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x19, 0x76, 0xA9, 0x14, 0x4B, 0x88, 0xC1, 0xD3, 0x87, 0x49, 0x08, 0x36, 0x57, 0x73, 0xA7, 0x65, 0xCD, 0xB0, 0x52, 0xC9, 0xEF, 0x5F, 0x1A, 0x80, 0x88, 0xAC, 0x98, 0xFB, 0x95, 0x64];
    ...
    fn test_04_correct_transaction_signing() {
        let account_old = Account::new(
            "Old",
            &[0x0A, 0x52, 0x65, 0x08, 0x2E, 0x24, 0x11, 0x5F, 0x77, 0x54, 0x0A, 0xB3, 0xB8, 0xC2, 0xB9, 0x20, 0x60, 0xAA, 0x30, 0xD6, 0xD2, 0xB8, 0x1A, 0x08, 0x5D, 0x71, 0xAB, 0x37, 0xED, 0xA7, 0x68, 0x91],
        ...
        let mut transaction_to_be_signed = Transaction::io_deserialize(&mut transaction_to_be_signed_bytes.as_slice()).unwrap();
        
        transaction_to_be_signed.get_signed_by_account(&account_old).unwrap();

        let signed_transaction_bytes: Vec<u8> = vec![];
        let signed_transaction = Transaction::io_deserialize(&mut signed_transaction_bytes.as_slice()).unwrap();
